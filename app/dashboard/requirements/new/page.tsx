...
interface Requirements/newItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function Requirements/newPage() {
  const [items, setItems] = useState<Requirements/newItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/requirements/new')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/requirements/new', {
      method: 'POST',
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="text-gray-600 mb-6">Create new compliance requirement tracking</p>
